...
export default class QueueManagerService {
  ...
  enqueue(interaction: ChatInputCommandInteraction, musics: Music[], appendNext: boolean): GuildQueueService {
    ...
    let queue = this.queues
      .find(it => it.guildId === (interaction.member as GuildMember).guild.id);
